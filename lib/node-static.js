...
Server = function (root, options) {
    ...
    if ('match' in this.options) {
        this.match = this.options.match;
    } else {
        this.match = '';
    }
    if ('replace' in this.options) {
...
Server.prototype.resolve = function (pathname) {
    pathname = this.rewrite(pathname);
    return path.resolve(path.join(this.root, pathname));
};

Server.prototype.rewrite = function (pathname) {
    if (this.match !== '' && pathname.match(this.match)) {
        pathname = pathname.replace(this.match, this.replace);
    }
    return pathname;
};
